...
import (
	...
	"time"
	"fmt"
	"github.com/smokinjack1559/aggregator/internal/database"
...
func (cfg *apiConfig) handlerFeedFollowCreate(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	fmt.Println(params, user.ID)
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Couldn't decode parameters")
		return
	}

	fmt.Println("1",uuid.New(),time.Now().UTC(),time.Now().UTC(),user.ID,params.FeedID)
	...
	fmt.Println(uuid.New(),time.Now().UTC(),time.Now().UTC(),user.ID,params.FeedID)
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Couldn't create feed follow")
		return
	}
